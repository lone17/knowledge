...
# Problem statement
In an n-dimensional space, given 2 vectors $x$ and $y$ with angle $\theta$, how to compute the rotation matrix that rotates $x$ to $y$ along the 2d subspace generated by $x$ and $y$ ?
...
- In the 2D plane generated by $x$ and $y$: 
	...
	- the projection in to this space is given by 
	  $$P = uu^T + vv^T$$
	- map $P$ to $\mathbb{R}^ 2$ and do the rotation there
		- $u$ and $v$ are 2 bases of a plane and $[u \; v]$ is the $\mathbb{R}^2$ mapping to that plane. ($\cdot [u \; v]$)
	...
	- then map back to n-dimensional ($\cdot [u \; v]^T$)
...
- The final rotation matrix is ^e160cf
  $$
  \begin{align}
  R &= I - (uu^T + vv^T) + [u \; v] \, R_\theta \,[u \; v]^T &(1) \\
  &= I + (uv^T - vu^T)sin(\theta) + (uu^T + vv^T)(cos(\theta) - 1) &(2)
  \end{align}
  $$
# Implementation
```python
