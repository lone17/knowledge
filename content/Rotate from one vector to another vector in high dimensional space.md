...
aliases: 
tags: 
modified: 2025-01-30 04:21 +07:00
created: 2024-11-28 17:31 +07:00
---
#maths/algebra 

# Problem statement
In an n-dimensional space, given 2 vectors $x$ and $y$ with angle $\theta$, how to compute the rotation matrix that rotates $u$ to $v$ along the 2d subspace generated by $x$ and $y$ ?
...
- In the 2D plane generated by $x$ and $y$: 
	...
	- mapping $P$ to $\mathbb{R}^ 2$ and do the rotation there
		- find 2 orthonormal vectors from $x$ and $y$ to represent that space
		  ...
		  \end{align}
		  $$
		  then $u$ and $v$ are 2 bases of a plane and $[u \; v]$ is the $\mathbb{R}^2$ mapping to that plane. ($\cdot [u \; v]$)
	- do the rotation with angle $\theta$ in that space ($\cdot R_\theta$)
	  ...
	  where $cos(\theta) = \frac{x \cdot y}{||x|| ||y||}$
		- then mapping back to n-dimensional ($\cdot [u \; v]^T$)
